//! Budget planning agent: multi-pipeline workflow on the MagickMind stack.
//!
        ...
        .on_message(move |ctx| {
            ...
            async move {
                ...
                let gate = match RelevanceGate::from_config(
                    "budget and cost planning — trip budgets, cost estimates, expenses, \
                     savings, income, financial goals, spending limits, or money management",
                    gate_llm,
                ) {
                    Ok(g) => g,
                    Err(e) => {
                ...
                // Relevance gate (invisible — halts if not budget-related)
