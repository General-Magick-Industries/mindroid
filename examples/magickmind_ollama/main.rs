//! Magick Mind infrastructure with Ollama as the LLM backend.
//!
...
//!     MINDROID_BASE_URL  — Base URL for MagickMind (e.g. https://api.magickmind.io)
...
use mindroid::pipeline::presets::magickmind::magickmind_ollama_pipeline;
...
async fn main() -> anyhow::Result<()> {
    ...
    let model = config.pipeline.model.as_deref().unwrap_or("smallthinker");

    let transport = CentrifugoTransport::new(ws_url, agent_id, identity.clone());
    let pipeline = magickmind_ollama_pipeline(identity.clone(), base_url, &ollama_url, model)?;
