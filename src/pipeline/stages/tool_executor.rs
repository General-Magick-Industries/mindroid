//! Native (JSON) tool calling — the default executor.
//!
//! The prompt-XML twin is [`XmlToolExecutorStage`](super::XmlToolExecutorStage),
//! kept for endpoints with no native `tools` field.
//!
...
//! [`LlmClient`](crate::llm_client::LlmClient)) and calls come
...
async fn artifact_turn(
    ...
    let dropped = super::tool_executor_xml::plan_reinjection(&mut load_ids);
    ...
    if !dropped.is_empty() {
        tracing::warn!(
            "ToolExecutorStage: re-attaching {} of {requested} requested artifacts",
            load_ids.len()
        );
...
async fn execute_local(
    ...
    let text = outcome.as_ref().unwrap_or_else(|e| e);
    debug!(
    ...
    async fn local_round(
        ...
        for call in &outcome.tool_calls {
            ...
            if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(&call.arguments)
                && let Some(id) = super::tool_executor_xml::get_artifact_id(&call.name, &parsed)
            {
                load_ids.push(id);
            }
...
mod tests {
    ...
    impl Tool for Deliver {
        ...
        async fn execute(&self, args: serde_json::Value, _: &ToolContext) -> Result<String> {
            args.get("msg")
                ...
                .ok_or_else(|| anyhow::anyhow!("msg is required").into())
        }
    ...
    async fn a_failed_turn_ending_call_loops_back() {
        let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
        let addr = listener.local_addr().unwrap();
        let first = deliver_round("{not json", "");
        ...
        assert_eq!(bodies.len(), 2, "the model gets to react to the error");
