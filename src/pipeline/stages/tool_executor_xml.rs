...
///
/// [`XmlToolExecutorStage`] performs this check itself so correlation cannot be
...
impl RemoteResultGate {
    ...
    /// stages living outside this module (e.g. `ToolExecutorStage`).
...
/// use mindroid::pipeline::stages::{SimpleContextBuilder, PostProcessor, XmlToolExecutorStage};
...
///     .add_streaming_stage(XmlToolExecutorStage::new(client, registry))
///     .add_stage(PostProcessor);
/// ```
#[derive(Clone)]
pub struct XmlToolExecutorStage {
    client: LlmClient,
...
#[async_trait]
impl PipelineStage for XmlToolExecutorStage {
    fn name(&self) -> &str {
    ...
    fn stream<'a>(&'a self, ctx: &'a mut Context) -> BoxStream<'a, StreamEvent> {
        ...
        Box::pin(async_stream::stream! {
            ...
            for iteration in 0..self.max_iterations {
                ...
                tracing::info!(
                    "XmlToolExecutorStage: found {} tool call(s): {:?}",
                    calls.len(),
                ...
                if iteration + 1 >= self.max_iterations {
                    warn!(
                        "XmlToolExecutorStage: reached max iterations ({}), stopping",
                        self.max_iterations
                    );
                    hit_max = true;
                    break;
