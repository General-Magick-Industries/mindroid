...
fn is_manifest_message(ctx: &Context) -> bool {
    ctx.message
        ...
        == Some(TOOL_MANIFEST_MESSAGE_TYPE)
}

/// Pipeline stage that applies a client's [`ToolsManifest`] to a
/// [`DynamicRegistry`]. A message declaring [`TOOL_MANIFEST_MESSAGE_TYPE`]
/// rebuilds the registry (keeping built-in local tools, replacing the remote
/// set) and halts the pipeline — the manifest is control traffic, not a turn for
/// the LLM. Place it before the tool executor.
///
/// Messages that merely contain manifest-shaped JSON are left alone: the type is
/// what makes a manifest, so quoting one in conversation is not advertising it.
///
/// # Trust
///
...
mod tests {
    ...
    async fn a_persistent_manifest_requires_authenticated_transport_identity() {
        ...
        message.metadata.insert(
            "message_type".into(),
            serde_json::Value::String(TOOL_MANIFEST_MESSAGE_TYPE.into()),
        );
        let mut ctx = Context::new(
            Arc::new(message),
            Arc::new(crate::config::AgentConfig::default()),
    ...
    /// The type is what makes a manifest — a body that merely looks like one is
    /// ordinary conversation and must reach the LLM untouched.
    #[tokio::test]
    async fn manifest_shaped_content_without_the_type_is_not_a_manifest() {
        let registry = DynamicRegistry::new(crate::tools::ToolRegistry::new());
        let stage = ManifestStage::new(registry.clone());
        let mut message = crate::models::Message::new(
            r#"{"type":"tools_manifest","payload":{"tools":[{"name":"remote"}]}}"#,
        ...
        assert!(!ctx.halted, "an ordinary message must keep flowing");
        assert!(registry.load().is_empty(), "registry must be untouched");
    }
