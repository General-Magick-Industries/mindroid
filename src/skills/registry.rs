...
//! 2. User skills directory (`~/.mindroid/skills/`) -- Trusted
//!
...
pub struct SkillRegistry {
    ...
    /// User skills directory (~/.mindroid/skills/).
    user_dir: PathBuf,
...
impl SkillRegistry {
    ...
    /// Register a built-in skill from embedded content (e.g., `include_str!`).
    ///
    /// Parses the content, validates it, and adds it as a `Bundled`+`Trusted`
    /// skill. Returns the skill name on success.
    pub fn register_builtin(&mut self, content: &str) -> Result<String, SkillRegistryError> {
        ...
        self.skills.push(skill);
        tracing::info!("Registered builtin skill: {}", name);
        Ok(name)
    }
    ...
    /// Call after `prepare_install_to_disk` completes.
    ...
    pub fn validate_remove(&self, name: &str) -> Result<PathBuf, SkillRegistryError> {
        ...
        match &skill.source {
            ...
            SkillSource::Bundled => Err(SkillRegistryError::CannotRemove {
                name: name.to_string(),
...
///
/// Shared implementation used by both discovery and installation. Avoids
/// duplicating the read/parse/validate/hash pipeline.
pub async fn load_and_validate_skill(
    path: &Path,
    trust: SkillTrust,
    source: SkillSource,
) -> Result<(String, LoadedSkill), SkillRegistryError> {
    ...
    // Check gating requirements (flat requires field on manifest)
...
mod tests {
    ...
    async fn test_discover_nonexistent_dir() {
        let mut registry = SkillRegistry::new(PathBuf::from("/nonexistent/mindroid/skills"));
        let loaded = registry.discover_all().await;
        assert!(loaded.is_empty());
    }
    ...
    async fn test_gating_failure_skips_skill() {
        ...
        // Uses flat `requires:` field (mindroid manifest structure)
        fs::write(
            skill_dir.join("SKILL.md"),
    ...
    #[tokio::test]
    async fn test_register_builtin_skill() {
        let dir = tempfile::tempdir().unwrap();
        let mut registry = SkillRegistry::new(dir.path().to_path_buf());
        ...
        assert!(registry.has("builtin-skill"));
        assert_eq!(registry.count(), 1);

        let skill = registry.find_by_name("builtin-skill").unwrap();
        assert_eq!(skill.trust, SkillTrust::Trusted);
        assert!(matches!(skill.source, SkillSource::Bundled));
    }

    #[tokio::test]
    async fn test_remove_bundled_skill_rejected() {
        let dir = tempfile::tempdir().unwrap();
        let mut registry = SkillRegistry::new(dir.path().to_path_buf());

        let content = "---\nname: bundled-skill\n---\n\nBundled prompt.\n";
        registry.register_builtin(content).unwrap();

        let result = registry.remove_skill("bundled-skill").await;
        assert!(matches!(
            result,
            Err(SkillRegistryError::CannotRemove { .. })
        ));
